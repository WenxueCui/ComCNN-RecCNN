function res = test_vl_simplenn(net, x, dzdy, res, varargin)
...
for i=1:n
  ...
  switch l.type
    case 'conv'
      ...
      % for the restraint for the sampling matrixes
      if(i  == -1)
         my_global = l.weights{1};
         % Sparsity control layer
         % l.weights{1} = Matrix_Sparse(0.95, l.weights{1}, []);
         % ¹éÒ»»¯¿ØÖÆ layer
         [l.weights{1},nums_1] = Binary2(l.weights{1},[]);
         global_1 = nums_1;
         net.layers{i}.weights{1} = l.weights{1};
      end
      ...
      % for the average of the pixels.
      if (i == -1)
         channels = size(res(i+1).x, 3);
         for k = 1:channels
             res(i+1).x(:,:,k,:) = res(i+1).x(:,:,k,:)/global_1(k);
         end
      end
    ...
    case 'elu'
      if l.leak > 0, leak = {'leak', l.leak} ; else leak = {} ; end
      res(i+1).x = vl_nnelu(res(i).x,[],leak{:}) ;
    ...
    case 'bcs_init_rec'
	  res(i+1).x = bcs_initialRec_wzhshi(res(i).x, l.dims);
      
    case 'JPEG'
      res(i+1).x = JPEG(res(i).x, l.QP);
      
    case 'HEVC'
      res(i+1).x = HEVC(res(i).x, l.QP);
      
    case 'JPEG2000'
      res(i+1).x = JPEG2000(res(i).x, l.QP);
      
    case 'BPG'
      res(i+1).x = BPG(res(i).x, l.QP);
  ...
  for i=n:-1:backPropLim
    ...
    switch l.type
        ...
        case 'conv'
            ...
            % the constraint for the sampling matrix.
            if(i == -1)
                dzdw{1} = gather(dzdw{1});
                maxs = max(max(max(max(abs(dzdw{1})))));
                maxs = maxs * 10;
                dzdw{1} = dzdw{1}/maxs;
                dzdx = Binary2(my_global,dzdw{1});
                dzdw{1} = dzdx;
                maxs = max(max(max(max(abs(dzdw{1})))));
                maxs = maxs * 10;
                dzdw{1} = dzdw{1}/maxs;
                net.layers{i}.weights{1} = my_global;
            end
     ...
     case 'elu'
      ...
      case 'bcs_init_rec'
	    res(i).dzdx = bcs_initialRec_wzhshi(res(i).x, l.dims,res(i+1).dzdx);

      case 'JPEG'
        res(i).dzdx = JPEG(res(i).x, l.QP, res(i+1).dzdx );
        
      case 'HEVC'
        res(i).dzdx = HEVC(res(i).x, l.QP, res(i+1).dzdx );
        
      case 'JPEG2000'
        res(i).dzdx = JPEG2000(res(i).x, l.QP, res(i+1).dzdx );
        
      case 'BPG'
        res(i).dzdx = BPG(res(i).x, l.QP, res(i+1).dzdx );
