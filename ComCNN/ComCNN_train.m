function [net, state] = CRCNN_train(net,net1, imdb, varargin)
...
for epoch = start+1 : opts.numEpochs
    ...
    [net, state] = process_epoch(net,net1, state, imdb, opts, 'train');
    [net,  ~   ] = process_epoch(net,net1, state, imdb, opts, 'test' );
    if numel(opts.gpus) == 1
...
%%%-------------------------------------------------------------------------
function  [net, state] = process_epoch(net,net1, state, imdb, opts, mode)
%%%-------------------------------------------------------------------------
...
for t=1:opts.batchSize:numel(subset)
    ...
    res = init_vl_simplenn(net,net1, inputs, dzdy, res, ...
        'mode', evalMode, ...
