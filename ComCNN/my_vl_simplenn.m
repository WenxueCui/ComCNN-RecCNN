...
for i=1:n
  ...
  switch l.type
	...
	% for the deparable conv.
	case 'dep_conv'
	  dims = size(res(i).x,3);
	  for dim = 1:dims
	     map_temp = res(i).x(:,:,dim,:);
		 res_temp = vl_nnconv(map_temp, l.weights{1}(:,:,dim,:), l.weights{2}(:,:,dim,:), ...
		    'pad', l.pad, ...
            'stride', l.stride, ...
            'dilate', l.dilate, ...
            l.opts{:}, ...
            cudnn{:}) ;
		 res(i+1).x(:,:,dim,:) = res_temp;
	  end
  ...
  for i=n:-1:backPropLim
    ...
    switch l.type
	  ...
	  case 'dep_conv'
	    ...
	    for dim = 1:dims
	       map_temp = res(i).x(:,:,dim,:);
		   [dzdx_temp, dzdw_temp1, dzdw_temp2] =  ...
		      vl_nnconv(map_temp, l.weights{1}(:,:,dim,:), l.weights{2}(:,:,dim,:), res(i+1).dzdx(:,:,dim,:), ...
		      'pad', l.pad, ...
              'stride', l.stride, ...
              'dilate', l.dilate, ...
              l.opts{:}, ...
              cudnn{:}) ;
		   res(i).dzdx(:,:,dim,:) = res_temp;
		   dzdw{1}(:,:,dim,:) = dzdw_temp1;
		   dzdw{2}(:,:,dim,:) = dzdw_temp2;
	    end
